                        ...
                        <div class="p-6 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-150">
                            <div class="flex items-start space-x-4">
                                ...
                                <div class="flex-shrink-0">
                                    @if($book->cover_image)
                                        <img src="{{ asset('storage/' . $book->cover_image) }}" alt="{{ $book->title }}" class="w-16 h-20 object-cover rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                                    @else
                                        <div class="w-16 h-20 bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-center border border-gray-200 dark:border-gray-700">
                                            ...
                                            @if($book->categories->count() > 0)
                                                <div class="flex flex-wrap gap-1 mt-3">
                                                    @foreach($book->categories->take(3) as $category)
                                                        <span class="inline-flex items-center px-2 py-1 text-xs bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300 rounded-md border border-gray-200 dark:border-gray-700">
                                                            {{ $category->name }}
                                                        </span>
                                                    @endforeach
                                                    @if($book->categories->count() > 3)
                                                        <span class="inline-flex items-center px-2 py-1 text-xs bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400 rounded-md border border-gray-200 dark:border-gray-700">
                                            ...
                                            <div class="flex items-center space-x-2">
                                                ...
                                                @php
                                                    $statusClasses = match($book->status) {
                                                        'available' => 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300 border border-green-200 dark:border-green-800',
                                                        'borrowed' => 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300 border border-yellow-200 dark:border-yellow-800',
                                                        'maintenance' => 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300 border border-orange-200 dark:border-orange-800',
                                                        default => 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 border border-red-200 dark:border-red-800'
                                                    };
                                                @endphp
                                            ...
                                            <div class="flex items-center space-x-2">
                                                ...
                                                @if(Auth::user()->isAdmin())
                                                    ...
                                                    <form action="{{ route('books.destroy', $book) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja excluir este livro?')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 p-1 rounded-md hover:bg-red-100 dark:hover:bg-red-900/20 transition-colors border border-transparent hover:border-red-200 dark:hover:border-red-800">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
