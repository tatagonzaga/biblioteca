    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Cadastrar Novo Livro</h1>
            </div>
            ...
            <form action="{{ route('books.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="publication_year" id="publication_year" value="{{ old('publication_year') }}" min="1000" max="{{ date('Y') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                        ...
                        <div>
                            ...
                            <select name="language" id="language" required
                                ...
                                <option value="de" {{ old('language') == 'de' ? 'selected' : '' }}>Alemão</option>
                                <option value="it" {{ old('language') == 'it' ? 'selected' : '' }}>Italiano</option>
                                <option value="ja" {{ old('language') == 'ja' ? 'selected' : '' }}>Japonês</option>
                                <option value="zh" {{ old('language') == 'zh' ? 'selected' : '' }}>Chinês</option>
                                <option value="ru" {{ old('language') == 'ru' ? 'selected' : '' }}>Russo</option>
                                <option value="other" {{ old('language') == 'other' ? 'selected' : '' }}>Outro</option>
                            </select>
                            @error('language')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label for="cover_image" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Imagem de Capa</label>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">PNG, JPG, JPEG ou GIF (máx. 2MB)</p>
                        @error('cover_image')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
